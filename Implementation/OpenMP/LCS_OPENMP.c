...
//Project Topic: OpenMP Implementation of Yang's Algorithm for Longest Common Subsequence problem
...
int main(int argc, char const **argv) {
    ...
    start_time = omp_get_wtime(); // strating the timer
...
#pragma omp parallel
{
   ...
   for (i = 1; i < input1_size + 1; i++) {
        ...
        for (j = 1; j < input2_size + 1; j++) {
            ...
            if (xi == yj) score(i);
            Score_table[index + j] = Score_table[index - width + j] + t * (s ^ 1); // if both characters match, then increase by score 1
        }
    }

    stop_time = omp_get_wtime();  // stopping the timer
    i = input1_size;
    j = input2_size;
    char xi, yj;
    ...
    //lcs reconstructing from the score table
    while(last_cell > 0) {
        int index = i * width + j;
        xi = input1[i - 1];
        yj = input2[j - 1];
