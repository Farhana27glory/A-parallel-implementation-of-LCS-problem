...
//Project Topic: MPI Implementation of Yang's Algorithm for Longest Common Subsequence problem
...
int main(int argc, char *argv) {
    ...
    for(k = 0; k < input2_per_proc; k++) {
        ...
        if (rank != first && block_size > 0) {
			...
			//receiving rows and columns of input strings from the root and other processors
        }
        ...
        if (block_size > 0 && block_high < input1_size) {
            ...
            if (rank == first) {
                MPI_Send(&Score_table[block_high * width + current_column], columns, MPI_INT, next, 0, MPI_COMM_WORLD); 
				// root sending each blocks of input 2 to the processors 
            } 
			
			else {
                MPI_Send(&Score_table[block_size * width + current_column], columns, MPI_INT, next, 0, MPI_COMM_WORLD);
				//other processors sending each element of input 1 to every processor
            }
        }
    }
